...
def copy_likes(ytm: Tuple[YTMusic, YTMusic]):
    ...
    liked_source_ids = functools.reduce(lambda l, i: l + [i['videoId']], liked_source['tracks'], [])

    print("\rLoading liked songs from destination account...", end="", flush=True)
    liked_dest = ytm[1].get_playlist("LM", limit=5000)
    liked_dest_ids = functools.reduce(lambda l, i: l + [i['videoId']], liked_dest['tracks'], [])
    ...
    if len(songs_to_like) < len(liked_source_ids):
        print(f"Skipping {len(liked_source_ids) - len(songs_to_like)} out of "
    ...
    try:
        for (index, song) in enumerate(songs_to_like):
            print(f"\rAdding songs to likes... {index + 1}/{len(songs_to_like)}", end="", flush=True)
...
def copy_playlist(ytm: Tuple[YTMusic, YTMusic], playlist_id: str, playlist_name: str = ""):
    ...
    song_ids = functools.reduce(lambda l, i: l + [i['videoId']], playlist_data['tracks'], [])

    print("Creating playlist... ", end="", flush=True)
    try:
        dest_playlist_id = ytm[1].create_playlist(playlist_data['title'],
                                                  playlist_data['description'] if playlist_data['description'] else "",
                                                  playlist_data['privacy'], song_ids)
        # dest_playlist_id = "TEST_IS_WORKING"
        if type(dest_playlist_id) == str:
            print(f"\rPlaylist created successfully! URL: https://music.youtube.com/playlist?list={dest_playlist_id}")
        else:
...
def parse_number_ids(selection: str):
    ...
    for token in id_tokens:
        try:
            ...
            print(f"Error: Invalid format for token '{token}'. Please use a valid format.")
            return None
    return result
...
def menu_copy_playlists(ytm: Tuple[YTMusic, YTMusic]):
    ...
    while True:
        sel = input("Selection (enter playlist numbers, 'A' for all, or 'C' to cancel): ")
        sel_playlists = []
